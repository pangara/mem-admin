...
module.exports = DBModel.extend ({
  ...
  plural : 'projects',
  sort: {name:1},
  populate: 'currentPhase phases phases.milestones phases.milestones.activities proponent primaryContact ownershipData.organization',
  ...
  // -------------------------------------------------------------------------
  //
  // promote, demote: major mines
  //
  // -------------------------------------------------------------------------
  promote: function(project, value) {
    project.isMajorMine = value;
    return this.saveAndReturn(project);
  },
  // -------------------------------------------------------------------------
  //
  ...
  published: function () {
    ...
    */
    var publishedProjects = new Promise(function(resolve, reject) {
      self.model.find ({ isPublished: true }, { _id: 1, code: 1, name: 1, region: 1, status: 1, eacDecision: 1, currentPhaseName: 1, currentPhase: 1, lat: 1, lon: 1, type: 1, description: 1, memPermitID: 1, isMajorMine: 1 })
        .sort ({ name: 1 })
